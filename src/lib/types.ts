...
// Anything a toast can render: plain text or a snippet (the Svelte counterpart of a ReactNode).
...
// Snippets are functions too, so a snippet result has to be returned from a function: `success: () => snippet`.
export type PromiseTResult<Data = any> =
	string | ((data: Data) => Renderable | Promise<Renderable>);
...
// `null` hides the icon of that type.
...
export interface ToastT {
	...
	// Set by `toast.custom()`, receives the id of the toast.
...
export function isAction(action: Action | Snippet): action is Action {
