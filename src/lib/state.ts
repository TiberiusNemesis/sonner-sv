...
class Observer {
	...
	message = (message: Renderable, data?: ExternalToast) => {
	...
	error = (message: Renderable, data?: ExternalToast) => {
		return this.create({ ...data, message, type: 'error' });
	};

	success = (message: Renderable, data?: ExternalToast) => {
		return this.create({ ...data, type: 'success', message });
	};

	info = (message: Renderable, data?: ExternalToast) => {
		return this.create({ ...data, type: 'info', message });
	};

	warning = (message: Renderable, data?: ExternalToast) => {
		return this.create({ ...data, type: 'warning', message });
	};

	loading = (message: Renderable, data?: ExternalToast) => {
	...
	promise = <ToastData>(promise: PromiseT<ToastData>, data: PromiseData<ToastData> = {}) => {
		...
		const settle = async (
			type: 'success' | 'error',
			option: PromiseTResult | PromiseTExtendedResult | undefined,
			value: unknown
		) => {
			shouldDismiss = false;
			const promiseData = typeof option === 'function' ? await option(value) : option;
			const description =
				typeof data.description === 'function' ? await data.description(value) : data.description;
	...
	custom = (jsx: NonNullable<ToastT['jsx']>, data?: ExternalToast) => {
		const id = getToastId(data);
...
const basicToast = (message: Renderable, data?: ExternalToast) => {
