...
import { toast } from '../src/lib/state.js';
...
test.describe('Basic functionality', () => {
	...
	test('toast is removed after swiping up', async ({ page }) => {
		await page.goto('/test/?position=top-left');
		await page.getByTestId('default-button').click();
		await page.hover('[data-sonner-toast]');
		await page.mouse.down();
		await page.mouse.move(0, -800);
	...
	test("toaster's theme should be dark", async ({ page }) => {
		await page.goto('/test/?theme=dark');
		await page.getByTestId('infinity-toast').click();
		await expect(page.locator('[data-sonner-toaster]')).toHaveAttribute(
			'data-sonner-theme',
			'dark'
	...
	test('Snippet description is rendered', async ({ page }) => {
		await page.getByTestId('snippet-description').click();
		await expect(page.getByText('This is my custom Snippet description')).toHaveCount(1);
	});
	...
	test('icons are hidden from assistive technology', async ({ page }) => {
		...
		await expect(page.locator('[data-sonner-toast] [data-close-button] svg')).toHaveAttribute(
			'aria-hidden',
			'true'
		);
	...
	test('a new toast reusing the id of a dismissed toast does not inherit its props', async ({
		page
	}) => {
	...
	test('description fills the width of the toast', async ({ page }) => {
		await page.getByTestId('snippet-description-wide').click();
